...
# UNetResNet34 model definition without masks
class UNetResNet34(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, dropout_prob=0.1):
        super(UNetResNet34, self).__init__()
        resnet = models.resnet34(weights=ResNet34_Weights.IMAGENET1K_V1)

        # Modify the first layer to accept in_channels (3 instead of 4)
    ...
    def forward(self, x):
        ...
        d1 = self.dropout(self.up1(e5))
        d2 = self.dropout(self.up2(torch.cat([d1, e4], dim=1)))
        d3 = self.dropout(self.up3(torch.cat([d2, e3], dim=1)))
        d4 = self.dropout(self.up4(torch.cat([d3, e2], dim=1)))
        output = self.conv_final(torch.cat([d4, e1_upsampled], dim=1))
...
# Wasserstein Loss with Gradient Penalty as the adversarial loss
class WassersteinLossGP:
    ...
    def compute_gradient_penalty(self, D, real_samples, fake_samples):
        ...
        gradients = torch.autograd.grad(outputs=d_interpolates, inputs=interpolates,
                                        grad_outputs=torch.ones(d_interpolates.size(), device=d_interpolates.device),
...
# Custom loss function that combines L1 and L2 losses
